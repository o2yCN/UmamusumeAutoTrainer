          ...
          <form>
            ...
            <div v-if="selectedUmamusumeTaskType?.id === 1">
                ...
                <div class="col-4">
                  <div class="form-group">
                    ...
                    <select v-model="selectedSupportCard" class="form-control" id="selectedSupportCard">
                      <option v-for="card in umausumeSupportCardList" :value="card">({{ card.desc }}) {{ card.name }}
                      </option>
                    </select>
                  </div>
                </div>
              ...
              <div class="form-group">
                ...
                <div class="form-group">
                  <span v-if="!showRaceList" class="btn auto-btn" style="width: 100%; background-color:#6c757d;"
                    v-on:click="switchRaceList">展开赛程选项</span>
                  <span v-if="showRaceList" class="btn auto-btn" style="width: 100%; background-color:#6c757d;"
                    v-on:click="switchRaceList">收起赛程选项</span>
                </div>
                  ...
                  <div class="col">
                    ...
                    <div class="form-check">
                      <div v-for="race in umamusumeRaceList_1">
                        <input class="form-check-input position-static" v-model="extraRace" type="checkbox" :id="race.id"
                          :value="race.id"><label :for="race.id"
                          v-if="race.type === 'GI' || race.type === 'GII' && !this.hideG2 || race.type === 'GIII' && !this.hideG3">
                          <span v-if="race.type === 'GIII'">&nbsp;<span style="background-color: #58C471;"
                              class="badge badge-pill badge-secondary">{{ race.type }}</span>&nbsp;</span>
                          <span v-if="race.type === 'GII'">&nbsp;<span style="background-color: #F75A86;"
                              class="badge badge-pill badge-secondary">{{ race.type }}</span>&nbsp;</span>
                          <span v-if="race.type === 'GI'">&nbsp;<span style="background-color: #3485E3;"
                              class="badge badge-pill badge-secondary">{{ race.type }}</span>&nbsp;</span>{{ race.date }}
                          {{ race.name }}</label>
                      </div>
                    </div>
                  </div>
              ...
              <div v-for="(item, index) in skillLearnPriorityList" :key="item.priority">
                <div class="form-group row">
                  ...
                  <div class="col-sm-6">
                    <textarea type="text" v-model="item.skills" class="form-control" id="skill-learn-priority"
                      placeholder="技能1名称,技能2名称,....(使用英文逗号)"></textarea>
                  </div>
                  <div class="col-sm-3">
                ...
                <div class="row">
                  <div class="col">
                    <div class="form-group">
                      ...
                      <textarea type="text" v-model="skillLearnBlacklist" class="form-control" id="skill-learn-blacklist"
                        placeholder="钢铁意志,迅疾如风,...(真不会有人点这些吧)"></textarea>
                    </div>
                  </div>
                </div>
              </div>
    ...
    addTask: function () {
      ...
      if (this.selectedUmamusumeTaskType.id === 1) {
        ...
        payload.attachment_data = {
          ...
          "stamina": this.opponentStamina
        }
      }
      else if (this.selectedUmamusumeTaskType.id === 3) {
        payload.attachment_data = {
          "shoe": this.askShoeType
        }
      }
